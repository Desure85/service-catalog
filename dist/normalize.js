...
import { z } from 'zod';
import { ApiImportV1Schema, ServiceSchema } from './types.js';
...
export function normalizeToV1(input) {
    ...
    if (typeof input === 'object' && input && 'items' in input) {
        const items = input.items;
        if (!Array.isArray(items))
            throw new Error('Invalid v0: items must be an array');
...
function normalizeServiceItem(it) {
    ...
    const svc = {
        ...
        endpoints: Array.isArray(it.endpoints) ? it.endpoints.map((e) => ({
            kind: e?.kind ?? 'http',
            url: String(e?.url),
