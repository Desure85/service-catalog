...
import { ApiImportV1Schema, QuerySchema, SCHEMA_VERSION, Service } from '../types.js';
...
export async function registerServiceRoutes(app: FastifyInstance, store: Store) {
  ...
  app.get('/api/services/:id', async (req, reply) => {
    ...
    if (!svc) return reply.code(404).send({ error: 'not_found', message: 'Service not found' });
    return svc;
  });
  ...
  app.post('/api/export-file', async (req, reply) => {
    ...
    if (!input.success) return reply.code(400).send({ error: 'invalid body', issues: input.error.issues });
    const { path, format, ...filters } = input.data as any;
    const items: Service[] = store.search({ ...filters });
    ...
    if (format === 'json') {
      ...
      const lines = items.map((it: Service) => JSON.stringify(it)).join('\n') + (items.length ? '\n' : '');
